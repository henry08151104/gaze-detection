...
function onResults(results) {
  ...
  if (results.multiFaceLandmarks) {
    ...
    const all_points = faceLandmarks.map((landmark) => [
      landmark.x * output_video.width,
      landmark.y * output_video.height,
    ]);

    // 將 all_points 傳到後端
    fetch("/dnn_process_landmarks", {
      method: "POST",
      headers: {
